...
class Navigator:
    ...
    def calculateMultiStart(self, navMap:NavMap, startLocationEndLetter, endLocationEndLetter, routeString):
        ...
        allAtEndLocation = False
        while allAtEndLocation == False:
            nextLocations = []
            for startLocation in startLocations:
                nextLocation = navMap.findNextLocation(startLocation, routeString[stepAmount % len(routeString)])
                nextLocations.append(nextLocation)
                #print('start ' + startLocation + ' next ' + nextLocation)
            startLocations = nextLocations
            stepAmount += 1
            print('** NEXT STEP ' + str(stepAmount))
            allAtEndLocation = len(list(startLocation for startLocation in startLocations if startLocation[-1] == endLocationEndLetter)) == len(startLocations)
